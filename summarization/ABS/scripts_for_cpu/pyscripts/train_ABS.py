...
for i in range(config.params.epoch):
    print('epoch: %d'%(i+1))
    accuracy = 0
    for j, n_slice_data, batch in reuters_dataset.make_batch(dataset, dictionary, config.params, seed):
        #print(batch)
        #print(batch['x_labels'])
        #print(batch['x_labels'].values.shape)
        #print(batch['x_labels'].values)
        #print(list(batch['x_labels'].values))
        # print(np.array(list(batch['x_labels'].values)))
