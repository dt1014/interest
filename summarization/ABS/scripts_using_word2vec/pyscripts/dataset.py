...
def make_batch(dataset, symbol_ids, params):
    ...
    pool = Pool(8)
    mp_results = pool.map(functools.partial(split_yc_t,
                                             window_size=window_size), dataset[['x_labels', 'yc_and_t_labels']].values)
    pool.close()
    print('done')
    
    for j, result in enumerate(mp_results):
        sys.stdout.write('\r   j: %10d (%10d)'%(j+1, len(mp_results)))
        if j == 0:
            results = result
        else:
    ...
    for i in range(n_batch):
        ...
        pool = Pool(8)
        mp_results = pool.map(functools.partial(pad_x,
